...
| v0.7.0 | [PRD-v0.7-test-execution-stack.md](./PRD-v0.7-test-execution-stack.md) | "EXPLAIN ANALYZE for tests" — span-based execution timeline + waterfall in viewer + `ulog explain` CLI | shipped (phase 1) | 380 |
...
| v0.9.0 | [PRD-v0.9-resource-validity.md](./PRD-v0.9-resource-validity.md) | Resource validity panel — viewer scans the project for JSON/TOML/CSV/INI files (YAML opt-in), reports parse status in a sidebar + `ulog validate-resources` CLI for CI. Stdlib parsers only. | shipped (CLI; panel deferred) | 337 |
| v0.10.0 | [PRD-v0.10-fleet-dashboard.md](./PRD-v0.10-fleet-dashboard.md) | Fleet dashboard for remote endpoint tests — `@probe(target=URL, parents=[...])` decorator + hierarchical fleet tree + per-target drill-down + cross-panel links to Tests / Authors panels. HTTP/HTTPS/TCP/Unix-socket probes via stdlib. | shipped (decorator; UI tree deferred) | 276 |
